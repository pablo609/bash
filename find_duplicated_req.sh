#!/usr/bin/bash
# This script finds duplicated requirements from a given list
# INPUT
# 1 - file with the list of requirements (copy and paste column C from Scenario Outline)
# OUTPUT
# 1 - prints the duplicated lines with the number of dupicates found

list=$(cat $1 | sed 's/\[.*\] //' | sed 's/;//' | sed 's/"//')
